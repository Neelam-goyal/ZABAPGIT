...
*                  company_code     TYPE string
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_nut1 TYPE string VALUE 'zsd_nut_tax_invoice/zsd_nut_tax_invoice'.
    CONSTANTS lc_template_nut2 TYPE string VALUE 'zsd_nut_tax_invoice2/zsd_nut_tax_invoice2'.
    CONSTANTS lc_template_nut3 TYPE string VALUE 'zsd_nut_tax_invoice3/zsd_nut_tax_invoice3'.
...
FIELDS A~BillingDocument , D~ShipToParty , E~BusinessPartnerFullName
WHERE A~BillingDocument = @bill_doc
...
LEFT JOIN i_CUSTOMERRETURNDELIVERYITEM AS C ON C~ReferenceSDDocument = B~ReferenceSDDocument
LEFT JOIN i_customerreturndelivery AS D ON D~CustomerReturnDelivery = C~CustomerReturnDelivery
LEFT JOIN i_customer AS E ON E~Customer = D~ShipToParty
LEFT JOIN i_address_2 AS F ON F~AddressID = E~AddressID
LEFT JOIN i_regiontext AS G ON G~Region = F~Region
LEFT JOIN I_CountryText AS H ON H~Country = F~Country
FIELDS A~BillingDocument , F~HouseNumber , F~StreetName , F~StreetPrefixName1 , F~StreetPrefixName2 ,
       F~StreetSuffixName1 , F~StreetSuffixName2 , F~CityName , G~RegionName , H~CountryName , F~PostalCode , E~TaxNumber3 , E~TelephoneNumber1
WHERE A~BillingDocument = @bill_doc
...
*    CONCATENATE lv_xml lv_supplier INTO lv_xml.
    ...
    ELSEIF printname = 'nut1'.
      lc_template_name = lc_template_nut1.
    ELSEIF printname = 'nut2'.
      lc_template_name = lc_template_nut2.
    ELSEIF printname = 'nut3'.
      lc_template_name = lc_template_nut3.
*
