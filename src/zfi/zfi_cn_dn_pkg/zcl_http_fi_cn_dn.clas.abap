...
CLASS ZCL_HTTP_FI_CN_DN  DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_HTTP_FI_CN_DN IMPLEMENTATION.
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZHTTP_FI_CN_DN?sap-client=080'.
    ...
    CASE request->get_method( ).
     ...
     DATA(lv_supplierinvoice) = request->get_form_field( `supplier_invoice` ).
       DATA(fs) = request->get_form_field( `fiscal_year` ).
        ...
        FIELDS supplierinvoice WHERE a~supplierinvoice = @lv_supplierinvoice and a~FiscalYear = @fs and a~CompanyCode = @cc
        INTO @DATA(lv_ac).

        IF lv_ac IS NOT INITIAL.

          TRY.
              DATA(pdf) = zcl_cn_dn_drv_class=>read_posts( supplier_invoice = lv_supplierinvoice Company_code = cc fiscal_year = fs ).
