CLASS zcl_http_outg_payment DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_HTTP_OUTG_PAYMENT IMPLEMENTATION.
...
*  |<form action="{ url }" method="POST">\n| &&
  |<form action="/sap/bc/http/sap/ZHTTP_OUTG_PAYMENT?sap-client=080" method="POST">\n| &&
  |<H2>BN Print</H2> \n| &&
...
*     req_uri = request->get_request_uri( ).
    ...
    req_uri = '/sap/bc/http/sap/ZHTTP_OUTG_PAYMENT?sap-client=080'.
    ...
    CASE request->get_method( ).
      ...
      WHEN CONV string( if_web_http_client=>post ).
        ...
        DATA(ac) = request->get_form_field( `belnr` ).            "( `lv_Accountingdocument` ).
...
*            response->set_text( pdf ).
  ...
  METHOD post_html.
   ...
   |<body> \n| &&
   |<title>Outgoing Payment Voucher</title> \n| &&
   |<form action="/sap/bc/http/sap/ZHTTP_OUTG_PAYMENT" method="Get">\n| &&
   |<H2>BN print</H2> \n| &&
