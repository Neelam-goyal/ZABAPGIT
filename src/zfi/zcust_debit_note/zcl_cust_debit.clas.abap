CLASS zcl_cust_debit DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS lc_template_name TYPE string VALUE 'ZFI_CUST_DEBIT/ZFI_CUST_DEBIT'.
ENDCLASS.



CLASS zcl_cust_debit IMPLEMENTATION.
   ...
   INTO @DATA(wa_head).
    ...
    FROM i_operationalacctgdocitem  WITH PRIVILEGED ACCESS AS a
      ...
      LEFT JOIN i_regiontext   WITH PRIVILEGED ACCESS AS d
        ON b~Region = d~Region AND d~Language = 'E' AND b~Country = d~Country
    WHERE  a~AccountingDocument = @accounting_no   AND a~CompanyCode = @Company_code AND a~FiscalYear = @fiscal_year
    ...
    SELECT SINGLE
          a~Customer,
           ...
           FROM I_OperationalAcctgDocItem AS a
            LEFT JOIN i_bupaidentification  WITH PRIVILEGED ACCESS AS b
            ON a~Customer = b~BusinessPartner
        WHERE  a~AccountingDocument = @accounting_no   AND a~CompanyCode = @Company_code AND a~FiscalYear = @fiscal_year
          AND a~FinancialAccountType = 'D'
          AND b~BPIdentificationType = 'FSSAI'
          INTO @DATA(bill_fssai).
...
*out->write( ship_to ).
    ...
    c~documentreferenceid,
      ...
      LEFT JOIN i_accountingdocumentjournal AS c ON a~AccountingDocument = c~AccountingDocument AND a~FiscalYear = c~FiscalYear
        WHERE a~AccountingDocument = @accounting_no   AND a~CompanyCode = @Company_code AND a~FiscalYear = @fiscal_year
...
*out->write( it_item ).
    ...
    a~TaxItemGroup
    WHERE a~AccountingDocument = @accounting_no   AND a~CompanyCode = @Company_code AND a~FiscalYear = @fiscal_year
    AND a~AmountInCompanyCodeCurrency < -1 AND a~AccountingDocumentItemType = 'T' AND
...
*out->write( it_sgst_amt ).
    ...
    SELECT FROM i_operationalacctgdocitem  AS a
        ...
        a~TaxItemGroup
        WHERE a~AccountingDocument = @accounting_no   AND a~CompanyCode = @Company_code AND a~FiscalYear = @fiscal_year
        AND a~transactiontypedetermination  = 'JOI'  AND a~AmountInCompanyCodeCurrency < -1 AND a~AccountingDocumentItemType = 'T'
...
*out->write( it_igst_amt ).
    ...
    DATA(lv_xml) =
       ...
       |<PhoneNumber2>{ wa_head_bill-TelephoneNumber2 }</PhoneNumber2>| &&
        ...
        |<stateName>{ wa_head_bill-RegionName }</stateName>| &&
         ...
         |<BPIdentificationNumber>{ bill_fssai-BPIdentificationNumber }</BPIdentificationNumber>| &&
         |</BillTo>| &&
         |<supplierDetails>| &&
       ...
       |<supp_PhoneNumber2>{ wa_head_ship-TelephoneNumber2 }</supp_PhoneNumber2>>| &&
        ...
        |<supp_stateName>{ wa_head_ship-RegionName }</supp_stateName>| &&
         ...
         |<BPIdentificationNumber>{ ship_fssai-BPIdentificationNumber }</BPIdentificationNumber>| &&
         |</supplierDetails>| &&
         |<invoiceNo>{ wa_head_invoice-DocumentReferenceID }</invoiceNo>| &&
    ...
    LOOP AT it_item INTO DATA(wa_line).
      ...
      AccountingDocument = wa_line-AccountingDocument taxitemgroup = wa_line-TaxItemGroup  CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear.

      DATA(lv_cgstAmt) =
|<cgstAmt>{ wa_camt-AmountInCompanyCodeCurrency }</cgstAmt>|.

      READ TABLE it_sgst_amt INTO DATA(wa_samt) WITH KEY AccountingDocument = wa_line-AccountingDocument  taxitemgroup = wa_line-TaxItemGroup CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear.

      DATA(lv_sgstAmt) =
              |<sgstAmt>{ wa_samt-AmountInCompanyCodeCurrency }</sgstAmt>|.


      READ TABLE it_igst_amt INTO DATA(wa_iamt) WITH KEY AccountingDocument = wa_line-AccountingDocument taxitemgroup = wa_line-TaxItemGroup CompanyCode = wa_line-CompanyCode  FiscalYear = wa_line-FiscalYear.
